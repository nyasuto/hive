...
from api import agents, instructions, tasks
    ...
    try:
        ...
        try:
            result = subprocess.run(
                ["tmux", "has-session", "-t", "beehive"], capture_output=True, timeout=5
            )
        ...
        if db_healthy:
            ...
            active_tasks_count = len(
                [t for t in tasks if t.status.value in ["pending", "in_progress"]]
            )
...
async def initialize_system(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if result["success"]:
            ...
            raise HTTPException(
                status_code=500, detail=f"System initialization failed: {result['stderr']}"
            )
...
async def stop_system(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if result["success"]:
            ...
            raise HTTPException(status_code=500, detail=f"System stop failed: {result['stderr']}")
