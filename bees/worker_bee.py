...
class WorkerBee(BaseBee):
    ...
    def __init__(self, bee_name: str, specialty: str, hive_db_path: str = "hive/hive_memory.db"):
        super().__init__(bee_name, hive_db_path)
        self.specialty = specialty  # "development", "qa", etc.
        ...
        self.logger.info(f"{specialty.title()} Worker Bee initialized - Ready for task execution")
    
    def _define_capabilities(self) -> List[str]:
        """Bee固有の能力を定義（サブクラスでオーバーライド）"""
        return ["general_work", "task_execution", "reporting"]
    
    def _update_bee_capabilities(self):
        """データベースでBeeの能力情報を更新"""
    ...
    def complete_task(self, task_id: int, result: str, deliverables: Optional[List[str]] = None,
                     work_summary: str = "", quality_notes: str = "") -> bool:
        """タスクを完了して結果を報告"""
        ...
        self.log_activity(task_id, "completed", f"Task completed by {self.bee_name}", {
    ...
    def report_progress(self, task_id: int, progress_percentage: int, status_note: str,
                       blocking_issues: Optional[List[str]] = None) -> bool:
        """作業進捗をQueenに報告"""
        if task_id != self.current_task_id:
...
"""
        ...
        if blocking_issues:
            progress_message += f"⚠️  Blocking Issues:\n"
        ...
        self.log_activity(task_id, "progress_update", 
    ...
    def request_assistance(self, task_id: int, assistance_type: str, details: str,
...
"""
        ...
        self.log_activity(task_id, "assistance_request", 
                         f"Requested {assistance_type}: {details}")
    ...
    def _generate_completion_report(self, task_details: Dict[str, Any], result: str, 
                                  deliverables: Optional[List[str]], work_summary: str,
                                  quality_notes: str, work_duration: float) -> str:
        """完了報告書を生成"""
...
"""
    ...
    def _handle_work_request(self, message: Dict[str, Any]):
        ...
        # 基本的には受け入れる
        from_bee = message['from_bee']
        response = f"Request acknowledged. I will prioritize this work."
    ...
    def _handle_queen_response(self, message: Dict[str, Any]):
        ...
        if task_id and task_id == self.current_task_id:
            # 指示に基づいて作業を調整
            self.log_activity(task_id, "guidance_received", 
                            f"Guidance from Queen: {message['content']}")
    ...
    def simulate_work(self, task_id: int, work_steps: List[Dict[str, Any]]) -> bool:
        ...
        deliverables = [f"Deliverable from {step.get('name', f'Step {i+1}')}" 
                       for i, step in enumerate(work_steps)]
        
        return self.complete_task(task_id, result, deliverables, 
                                "Simulated work execution", "All steps completed as planned")